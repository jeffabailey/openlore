...
| `openlore` CLI `ui` verb (the read-only viewer; `GET /` `/claims` `/peer-claims` `/scrape` `/search` `/score` `/static/htmx.min.js`) | long-running subprocess `openlore ui --port 0` via `assert_cmd::cargo_bin("openlore")`, bound ephemeral `:0` (read back from `viewer.serve.listening`), driven over HTTP (`ViewerServer::get`/`get_htmx`/`post_form`); HX-Request fork | slice-06/07/08/09; loopback-only; holds NO signing key; slice-08 adds `GET /search`; slice-09 adds `GET /score` (LOCAL read + PURE `scoring::score`, NO network — distinct from /scrape + /search) |
