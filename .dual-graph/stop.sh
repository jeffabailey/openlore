...
if input_tokens > 0 or cache_create > 0 or cache_read > 0 or output_tokens > 0:
    print(json.dumps({
        ...
        "description": "auto",
        "project": "/Users/jeffbailey/Projects/foss/leading/openlore",
    }))
PYEOF
)
  if [[ -n "$USAGE" ]]; then
    # POST to MCP graph server (always running, reliable)
    MCP_PORT=$(cat "/Users/jeffbailey/Projects/foss/leading/openlore/.dual-graph/run/claude/mcp_port" 2>/dev/null || echo "8080")
