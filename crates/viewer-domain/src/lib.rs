...
pub const PEER_ADD_GUIDANCE_PREFIX: &str =
    "Subscribe to a peer from the CLI: openlore peer add";
...
mod tests {
    ...
    fn render_peers_fragment_renders_one_attributed_row_per_peer_with_its_own_count() {
        ...
        assert!(html.contains(PEERS_REGION_ID), "fragment must carry the #peers region id; {html}");
        // Each peer DID is rendered verbatim with its OWN per-peer count.
        assert!(html.contains("did:plc:rachel-test"), "Rachel's DID must render verbatim; {html}");
        assert!(html.contains("did:plc:tobias-test"), "Tobias's DID must render verbatim; {html}");
        assert!(html.contains("5 cached claims"), "Rachel's per-peer count 5 must render; {html}");
        assert!(html.contains("3 cached claims"), "Tobias's per-peer count 3 must render; {html}");
        ...
        for banned in ["all peers", "consensus", "combined total"] {
            assert!(!lowered.contains(banned), "no merged {banned:?} aggregate row; {html}");
        }
    }
    ...
    fn render_peers_fragment_renders_a_zero_claims_peer_at_count_zero() {
        ...
        let html = render_peers_fragment(&view).into_string();
        assert!(html.contains("did:plc:newpeer-test"), "the zero-claims peer must render; {html}");
        assert!(html.contains("0 cached claims"), "the zero-claims peer must render count 0; {html}");
    }
    ...
    fn render_remove_guidance_emits_the_bare_did_revocation_command_as_text() {
        ...
        let lowered = html.to_ascii_lowercase();
        for banned in ["<button", "<form", "hx-post", "hx-delete", "hx-put", "name=\"remove\""] {
            assert!(!lowered.contains(banned), "the command must be render-only TEXT, no {banned:?}; {html}");
        }
    }
    ...
    fn render_peers_fragment_no_subscriptions_renders_guided_empty_state() {
        ...
        assert!(html.contains(PEERS_REGION_ID), "the empty-state fragment carries the #peers id; {html}");
        assert!(
    ...
    fn render_peers_page_embeds_the_fragment_and_carries_offline_chrome() {
        ...
        assert!(page.contains(&fragment), "the full page must EMBED the fragment verbatim (parity); page:\n{page}");
        assert!(page.to_ascii_lowercase().contains("<!doctype html>"), "the page must be a full document; {page}");
        assert!(page.contains(HTMX_ASSET_URL), "the page must reference the LOCAL htmx asset (offline-first); {page}");
        assert!(
